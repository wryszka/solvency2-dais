  ...
  return (
    <section className={`bg-white border-2 ${colour.border} rounded-xl overflow-hidden flex flex-col`}>
      <header className={`${colour.accent} px-4 py-3 border-b ${colour.border}`}>
        ...
        <Link to={`/pillar-${col.pillar}`}
          state={{ crumbs: [{ label: 'Reporting Cycle', to: '/reporting-cycle' }, { label: `Pillar ${col.pillar} — overview` }] }}
          className={`mt-2 inline-flex items-center gap-1 text-[11px] font-bold ${colour.text} hover:underline`}>
          {col.pillar === 1 && 'Open Pillar 1 overview — architecture, engines, controls'}
          {col.pillar === 2 && 'Open Pillar 2 overview — three lines, governance, audit'}
          {col.pillar === 3 && 'Open Pillar 3 overview — calendar, sign-off, submission'}
